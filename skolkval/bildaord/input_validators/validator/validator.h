...
[[noreturn]] void die_line(const string &msg);
...
void die_line(const string &msg) {
    ...
    else if (_consumed_lineno == -1)
        die(msg + " (before reading any input)");
    else
        die(msg + " on line " + to_string(_consumed_lineno));
}
